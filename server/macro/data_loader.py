...
class GlobalMacroLoader:
    ...
    def calculate_rsrs(self, prices: pd.Series, window: int = 18) -> float:
        ...
        # 使用 astype 确保数据类型是浮点数，避免类型检查问题
        y = prices.iloc[-window:].values.astype(np.float64)
        x = np.arange(len(y), dtype=np.float64)
        
        # 使用 numpy.polyfit 计算线性回归，添加类型忽略标记
        # polyfit 返回多项式系数，对于线性回归是一阶多项式
        coefficients = np.polyfit(x, y, 1)  # type: ignore # deg=1 表示线性回归
        slope = coefficients[0]
        
        # 计算 R² (决定系数)
        y_pred = np.polyval(coefficients, x)
        ss_res = np.sum((y - y_pred) ** 2)
        y_mean = np.mean(y)  # type: ignore
