...
class MacroConfig:
    """
    宏观策略配置类 (JSON 驱动版 - 适配MY-DOGE-MICRO项目结构)
    """
    ...
    # 扫描器配置 (从models_config.json继承)
    scanner_filters: Optional[dict] = None
    ...
    def _load_from_json(self):
        """加载配置文件 - 适配MY-DOGE-MICRO项目结构"""
        # 智能定位 JSON 文件 (相对于项目根目录)
        project_root = Path(__file__).parent.parent.parent
        json_path = project_root / 'config' / 'models_config.json'
        
        if not json_path.exists():
            # 尝试相对路径
            json_path = Path("config/models_config.json")
        ...
        try:
            ...
            # 5. 加载扫描器过滤器配置
            self.scanner_filters = data.get('scanner_filters', {})
    ...
    def get_asset_symbols(self) -> list:
        """获取所有资产符号列表"""
        symbols = [
            self.tech_proxy,
            self.safe_haven_proxy,
            self.target_asset
        ]
        if self.crypto_proxy:
            symbols.append(self.crypto_proxy)
        return symbols

    def get_asset_names(self) -> dict:
        """获取资产符号到名称的映射"""
        return {
            self.tech_proxy: self.tech_name,
            self.safe_haven_proxy: self.safe_name,
            self.target_asset: self.target_name,
            self.crypto_proxy: self.crypto_name
        }
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "assets": {
                "tech": {"symbol": self.tech_proxy, "name": self.tech_name},
                "safe": {"symbol": self.safe_haven_proxy, "name": self.safe_name},
                "crypto": {"symbol": self.crypto_proxy, "name": self.crypto_name},
                "target": {"symbol": self.target_asset, "name": self.target_name}
            },
            "macro_settings": {
