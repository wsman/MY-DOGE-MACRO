...
export interface UseWebSocketReturn {
  ...
  stats: {
    ...
    latencyMs: number | null;  // RTT延迟（毫秒）
  };
  /** 订阅指定 ticker */
      ...
      switch (message.type) {
        case 'price_update':
          if (message.ticker && message.data) {
            ...
            const { price, change, volume, high, low, open, previousClose } = message.data;
            setMarketData(message.ticker, {
              ticker: message.ticker,
              name: message.ticker, // 实际应用中应从其他地方获取名称
              price,
              change,
              changePercent: message.data.changePercent || (change / price) * 100,
              volume,
