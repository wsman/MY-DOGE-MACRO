      ...
      this.client.interceptors.response.use(
        (response: any) => {
          ...
          if (requestTime) {
            ...
            console.log(
              `📡 ${response.config.method?.toUpperCase()} ${response.config.url} - ${response.status} (${latency}ms)`
            );
          }
          return response;
        },
...
export const marketApi = {
  ...
  async getSnapshot() {
    const response = await apiClient.get<{ columns: string[]; data: any[][] }>(
  ...
  async getKline(ticker: string, limit: number = 500) {
    const response = await apiClient.get<{ columns: string[]; data: any[][] }>(
      `/api/v1/market/kline/${ticker}?limit=${limit}`
    );
...
export const analysisApi = {
  ...
  async calculateVolatilitySkew(ticker: string, shortPeriod: number = 5, longPeriod: number = 20) {
    return apiClient.get<any>(
      `/api/v1/analysis/volatility/${ticker}?short=${shortPeriod}&long=${longPeriod}`
    );
  },
