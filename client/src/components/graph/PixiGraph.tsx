...
const PixiGraphInner = (props: PixiGraphProps) => {
  ...
  const createNodes = useCallback((app: Application, nodes: PixiGraphProps['data']['nodes']) => {
  ...
  useEffect(() => {
    ...
    async function init() {
      if (!containerRef.current || cleanupDone) return;
      ...
      // 获取数据
      const nodes = props.data?.nodes || Array.from({ length: 1000 }, (_, i) => ({
        id: `n${i}`,
        name: `Node ${i}`,
        x: Math.random() * 800,
        y: Math.random() * 600,
      ...
      const links = props.data?.links || Array.from({ length: 999 }, (_, i) => ({
        source: `n${i}`,
        target: `n${i+1}`,
        strength: 1.0
      }));
