...
| 条款 | 公理名称 | 核心定义 | 项目状态 |
|------|----------|----------|----------|
| **§100** | 宪法同步公理 | 版本变更必须触发全体系同步扫描与强制对齐 | ✅ 当前v2.0.0-alpha |
| **§101** | 功能分层拓扑公理 | 系统严格遵循T0-T3分层架构，层级间单向依赖 | ✅ T0-T3已实现 |
| **§102** | 双存储同构公理 | 内存状态必须与文件系统状态一致 ($S_{runtime} \equiv S_{disk}$) | ✅ Qdrant+文件系统 |
| **§103** | 编码一致性公理 | 禁止热更新，启动过程必须确定且可重复 | ✅ UTF-8原子写入 |
| **§104** | 数据完整性公理 | 所有状态变更必须是原子的，且经过完整性校验 | ✅ 原子操作协议 |
| **§105** | MCP微内核神圣公理 | MCP工具是系统与外界交互的唯一合法途径 | ✅ MCP服务已部署 |
| **§106** | 熵减验证公理 | 重构必须满足语义保持性 ($S' = S$) 和熵减验证 ($H' \leq H$) | ✅ 审计协议 |
| **§107** | 单一真理源公理 | `memory_bank` 是可执行规范的唯一真理源，引用而非复制 | ✅ 本文件为真理源 |
| **§108** | T3文档结构公理 | `memory_bank/t3_documentation/` 必须保持扁平结构，禁止子目录 | ✅ 扁平结构已实施 |
| **§110** | 用户主权公理 | AI是执行器，人类用户拥有最终决策权与否决权 | ✅ CDD用户批准 |
| **§111** | 上下文单一存储公理 | 活动上下文 (`activeContext`) 必须存储在单一位置 | ✅ `active_context.md` |
| **§112** | 上下文时效性公理 | activeContext保留最近3天内关键事件概要，早期事件迁移至T1 | ✅ 自动归档机制 |
| **§113** | 渲染复杂度公理 | 严格限制单次渲染的Token数量，优先使用摘要与索引 | ✅ 摘要优先策略 |
| **§114** | 类型公理优先原则 | 类型定义 (.d.ts) 先于代码实现 ($T_{define} \rightarrow T_{implement}$) | ✅ TypeScript严格模式 |
| **§115** | 网络韧性公理 | 通信必须具备熔断、重试与降级机制 | ✅ G.U.A.R.D.弹性通信 |
| **§116** | 实时通信公理 | WebSocket连接必须通过websocket_hub.WebSocketHub统一管理，支持进度追踪和频道订阅 | ✅ websocket_hub集成 |
| **§117** | 模型选择器公理 | 必须根据任务复杂度动态选择最优模型以平衡成本与性能 | ✅ ModernModelSelector |
| **§118** | 容器化部署公理 | 所有生产级项目必须支持Docker容器化部署，提供Dockerfile、docker-compose.yml和健康检查 | ✅ MY-DOGE-MACRO v2.0.0-alpha Docker化 |
| **§119** | 共享库治理公理 | 共享库必须独立可测试、独立版本化、独立文档化，遵循语义化版本(SemVer)，不得有循环依赖 | ✅ shared-libs架构 |
| **§120** | 交叉移植公理 | 功能应优先从成熟项目移植到新项目，而非重复开发，建立跨项目技术共享机制 | ✅ 共享库交叉移植 |
| **§121** | 多模型协同公理 | 所有AI模型调用通过ai_adapters统一接口，支持智能路由基于任务类型、成本、延迟选择模型，支持故障转移 | ✅ ai_adapters集成 |
| **§122** | 安全公理 | 检查依赖包存在性，使用参数化查询防御注入 | ✅ 依赖检查+参数化查询 |
